...
$(document).ready(function () {
    ...
    function initialize() {
        map = new google.maps.Map(document.getElementById('map'), {
            zoom: 18,
            scrollwheel: false,
            center: {lat: -12.102501, lng: -77.022063}
        });

        // marker
        var marker = new google.maps.Marker({
        map: map,
        position: {lat: -12.102501, lng: -77.022063},
        title: 'Doapps SAC'
      });
    ...
    $(document).on("submit", "form#formReservation", function() {
        if(!validation($(this).attr("id"))) {
            //here code if validation is successful
            var name = document.getElementsByName("u_name")[0].value;
            var phone = document.getElementsByName("u_phone")[0].value;
            var mail = document.getElementsByName("u_mail")[0].value;
            var text = document.getElementsByName("u_text")[0].value;
            console.log("holi"+name+","+phone+","+mail+","+text);
            ...
            dataType: 'json',
                ...
                success: function(data){
                    if (data.success == 1) 
                    {
                        ...
                        document.getElementsByName("u_phone")[0].value = "";
                        document.getElementsByName("u_mail")[0].value = "";
                        document.getElementsByName("u_text")[0].value = "";
                    }
                    else{
                        console.log(data.message);
    ...
    $('#sendButton').on('click', function(event){
        //$('#reservation').hide();
        if(!validation("formReservation-2")) {
            //here code if validation is successful
            var name = $("#u_name").val();
